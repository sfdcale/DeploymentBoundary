...
let format = require('./fileFormats');

function dependencyApi(connection, metadataId) {
    let toolingApi = connection.tooling;
    ...
    async function getDependencies() {
        ...
        let unsupportedDependencies = await createUnsupportedDependencies(
            dependencies
        );
    ...
    async function getDependentPackages(dependencies) {
        ...
        let query =
            'select id, SubscriberPackage.Name, SubscriberPackage.NamespacePrefix from InstalledSubscriberPackage';
                    ...
                    pills: [
                        {
                            ...
                            type: 'warning',
                            description:
                                'This package must be installed in the destination org before the deployment boundary can be deployed'
                        }
                    ]
                };
    ...
    function initTree(dependencies) {
        let dependencyTree = dependencies.reduce((tree, dep) => {
            ...
            if (!tree[newNodeKey]) {
                ...
                //if the tree already has a node for this key
                ...
                if (tree[newNodeKey].references[dep.type]) {
                    ...
                    let listOfReferencedNames = tree[newNodeKey].references[
                        dep.type
                    ].map((dep) => dep.name);
                    ...
                    // if the tree has a node for this key, but it doesn't yet have a property for this metadata type
                    //we add it here
    ...
    function reorderNodes(tree) {
        for (let currentNodeKey in tree) {
            ...
            for (let anyNodeKey in tree) {
                ...
                let anyNodeReferencesCurrentNode =
                    anyNode.refNames.indexOf(currentNodeKey) != -1;
    ...
    function recursiveDependencyQuery() {
        ...
        async function exec(ids) {
            ...
            let rawResults = await toolingApi.query(soqlQuery.query);
    ...
    function simplifyResults(rawDependencies) {
        ...
        let dependencies = rawDependencies.records.map((dep) => {
            let simplified = {
                ...
                repeated: false,
                url: `${connection.instanceUrl}/${dep.RefMetadataComponentId}`,
                notes: null,
        ...
        dependencies.forEach((dep) => {
            if (utils.isDynamicReference(dep)) {
                dep.nots =
                    'This entity is dynamically referenced in Apex so its value can only be determined at run time. You may need to manually deploy its components to the target org.';
    ...
    async function enhanceCustomFieldData(dependencies) {
        ...
        if (customFieldIds.length) {
            ...
            let objectIdsByCustomFieldId = await getFieldToEntityMap(
                customFieldIds
            );

            dependencies.forEach((dep) => {
                if (isCustomField(dep.type)) {
                    dep.name = getCorrectFieldName(
                        dep.name,
                        dep.id,
                        objectIdsByCustomFieldId,
                        objectNamesById
                    );
                }
                ...
                if (isCustomField(dep.referencedBy.type)) {
                    dep.referencedBy.name = getCorrectFieldName(
    ...
    async function createUnsupportedDependencies(dependencies) {
        ...
        if (customFieldNames.length) {
            ...
            records = await mdapi.read('CustomField', customFieldNames);
        }
        ...
        let picklistsWithValueSet = records.filter(
            (rec) => rec.valueSet && rec.valueSet.valueSetName
        );
        let dependentPicklists = records.filter(
            (rec) => rec.valueSet && rec.valueSet.controllingField
        );

        if (lookupFields.length) {
            let lookupFieldsDependencies = await createLookupFieldDependencies(
                lookupFields,
                customFieldsByName
            );
        ...
        if (picklistsWithValueSet.length) {
            let valueSetDependencies = createValueSetDependencies(
                picklistsWithValueSet,
                customFieldsByName
            );
        ...
        if (dependentPicklists.length) {
            let controllingPicklists = createControllingPicklistDependencies(
                dependentPicklists,
                customFieldsByName
            );
    ...
    async function createLookupFieldDependencies(
        lookupFields,
        customFieldsByName
    ) {
        ...
        let customObjectIdsByName = await utils.getObjectIdsByName(
            connection,
            cache
        );
        ...
        for (let fieldName of customFieldsByName.keys()) {
            if (lookupFieldsByName.has(fieldName)) {
                ...
                let relatedObjectId =
                    customObjectIdsByName.get(relatedObjectName);
    ...
    function createValueSetDependencies(
        picklistsWithValueSet,
        customFieldsByName
    ) {
        let picklistsByName = new Map();
        picklistsWithValueSet.forEach((vs) =>
            picklistsByName.set(vs.fullName, vs)
        );
    ...
    function createControllingPicklistDependencies(
        dependentPicklists,
        customFieldsByName
    ) {
        let dependentPicklistsByName = new Map();
        dependentPicklists.forEach((dp) =>
            dependentPicklistsByName.set(dp.fullName, dp)
        );
        ...
        for (let fieldName of customFieldsByName.keys()) {
            if (dependentPicklistsByName.has(fieldName)) {
                let dependentPicklistInfo =
                    dependentPicklistsByName.get(fieldName);
                let fieldId = customFieldsByName.get(fieldName).id;
    ...
    async function getFieldToEntityMap(customFieldIds) {
        ...
        let results = await toolingApi.query(soqlQuery.query);
